...
export default function DemoRequestForm({
  ...
  return (
    <form
      ...
      className={`border border-white/10 bg-[#060a15]/90 p-5 sm:p-7 ${className}`}
    >
      ...
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-cyan-300">Architecture consultation intake</p>
        <h2 className="mt-3 text-2xl font-semibold tracking-tight text-white sm:text-3xl">Tell us what you are trying to build, secure, govern, or modernize.</h2>
        <p id="consultation-form-guidance" className="mt-3 text-sm leading-6 text-slate-400">Give us enough context to prepare for a useful architecture conversation. Fields marked with an asterisk are required.</p>
        {validDefaultArea ? <p className="mt-4 border-l-2 border-cyan-300/60 pl-3 text-sm leading-6 text-slate-300">We carried forward the consultation context from the page you were reviewing. You can change the selected area below.</p> : null}
      </div>

      <div className="mt-7 grid gap-5">
        <div className="grid gap-5 sm:grid-cols-2"><InputField label="Full name" name="fullName" required autoComplete="name" /><InputField label="Work email" name="email" type="email" required autoComplete="email" /></div>
        <div className="grid gap-5 sm:grid-cols-2"><InputField label="Company" name="company" required autoComplete="organization" /><InputField label="Role / title" name="role" autoComplete="organization-title" /></div>

        <label className="grid gap-2 text-sm font-medium text-white/90"><span>Your role in the initiative<span className="ml-1 text-cyan-300" aria-hidden="true">*</span></span><select required aria-required="true" name="buyerRole" defaultValue="" className={selectClassName}><option value="" disabled>Select your role</option>{buyerRoles.map((role) => <option key={role} value={role}>{role}</option>)}</select></label>
        <label className="grid gap-2 text-sm font-medium text-white/90"><span>Primary consultation area<span className="ml-1 text-cyan-300" aria-hidden="true">*</span></span><select required aria-required="true" name="consultationArea" defaultValue={validDefaultArea} className={selectClassName}><option value="" disabled>Select an area</option>{consultationAreas.map((area) => <option key={area} value={area}>{area}</option>)}</select></label>
        <label className="grid gap-2 text-sm font-medium text-white/90"><span>Timeline<span className="ml-1 text-cyan-300" aria-hidden="true">*</span></span><select required aria-required="true" name="timeline" defaultValue="" className={selectClassName}><option value="" disabled>Select timeline</option>{timelines.map((timeline) => <option key={timeline} value={timeline}>{timeline}</option>)}</select></label>
        ...
        <button type="submit" disabled={isSubmitting} className="mt-1 inline-flex min-h-12 w-full items-center justify-center rounded-md bg-cyan-300 px-6 py-3 text-sm font-semibold text-[#021018] hover:bg-cyan-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-2 focus-visible:ring-offset-[#060a15] disabled:cursor-not-allowed disabled:opacity-70">{isSubmitting ? "Submitting..." : "Request Architecture Consultation"}</button>

        <div className="border-t border-white/10 pt-4 text-xs leading-5 text-slate-400"><p className="font-semibold text-slate-300">Share business context, not sensitive data.</p><p className="mt-1">Do not include passwords, credentials, protected health information, customer records, secrets, or highly sensitive production data. We only need enough context to prepare for the first conversation.</p></div>
      </div>
    </form>
  );
}
