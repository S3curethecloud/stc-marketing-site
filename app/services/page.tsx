...
const engagementDepth = [
  {
    ...
    body: "Clarify priorities, tradeoffs, risk, operating constraints, and investment direction before committing to an architecture path.",
  },
  {
    title: "Architecture definition",
    body: "Define target-state patterns, boundaries, control models, design decisions, and cross-functional responsibilities.",
  },
  {
    title: "Implementation-ready engineering",
    body: "Translate architecture into ADRs, control specifications, backlog guidance, integration patterns, and delivery artifacts.",
  },
] as const;
...
const services = [
  {
    ...
    related: [["AI security architecture", "/ai-security-architecture"], ["Secure RAG", "/secure-rag"], ["Agentic AI security", "/agentic-ai-security"]],
  },
  {
    id: "cloud-governance",
    ...
    related: [["Cloud security", "/cloud-security"], ["Platform security", "/platform-security"], ["Services overview", "/services"]],
  },
  {
    ...
    related: [["AI governance", "/ai-governance"], ["AI use cases", "/ai-use-cases"], ["Insights", "/insights"]],
  },
  {
    ...
    related: [["AI governance", "/ai-governance"], ["AI security", "/ai-security"], ["Insights", "/insights"]],
  },
  {
    id: "regulated-ai",
    ...
    related: [["Healthcare AI", "/healthcare-ai"], ["Regulated AI", "/regulated-ai"], ["Industries", "/industries"]],
  },
  {
    ...
    related: [["Company", "/company"], ["Insights", "/insights"], ["Request consultation", "/request-demo"]],
  },
] as const;
      ...
      <section className="border-b border-white/10 bg-[#030711]">
        <Container className="py-16 sm:py-20 lg:py-24">
          <div className="max-w-5xl">
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-300">Enterprise services</p>
            <h1 className="mt-5 text-[2.4rem] font-semibold leading-[1.05] tracking-[-0.035em] text-white sm:text-5xl lg:text-[3.5rem]">
              Architecture services for secure AI programs operating under real constraints.
            </h1>
            <p className="mt-6 max-w-3xl text-base leading-7 text-slate-300 sm:text-lg sm:leading-8">
              We help senior technology, security, cloud, AI, and governance leaders turn ambiguous requirements into clear architecture decisions, control models, and implementation-ready direction.
            </p>
            <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center">
              <Link href="/request-demo" className="inline-flex min-h-12 items-center justify-center bg-cyan-300 px-6 py-3 text-sm font-semibold text-slate-950 hover:bg-cyan-200">
      ...
      <section className="border-b border-white/10 bg-[#050816]">
        <Container className="py-14 sm:py-16">
          <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-16">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Engagement depth</p>
              <h2 className="mt-4 text-3xl font-semibold tracking-[-0.025em] text-white sm:text-4xl">Not every engagement needs the same level of intervention.</h2>
              <p className="mt-5 text-base leading-7 text-slate-400">We scale from decision support to implementation-ready engineering based on the problem, team maturity, risk, and delivery context.</p>
            </div>
            <div className="border-t border-white/10">
              {engagementDepth.map((item) => (
      ...
      <section id="services" className="bg-[#030711]">
        <Container className="py-14 sm:py-16 lg:py-20">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Service architecture</p>
            <h2 className="mt-4 text-3xl font-semibold tracking-[-0.025em] text-white sm:text-4xl">Know when to engage us, what we will work through, and what your teams will receive.</h2>
          </div>
              ...
              <article id={service.id} key={service.id} className="border-b border-white/10 py-10 sm:py-12">
                <div className="grid gap-10 lg:grid-cols-[0.82fr_1.18fr] lg:gap-16">
                  <div>
                    ...
                    <Link href="/request-demo" className="mt-6 inline-flex text-sm font-semibold text-cyan-300 hover:text-cyan-200">Request architecture consultation -&gt;</Link>
                  </div>

                  <div className="grid gap-8 md:grid-cols-2">
                    <div>
                      <h4 className="text-sm font-semibold text-white">This is for you when</h4>
                      <ul className="mt-4 space-y-3 text-sm leading-6 text-slate-400">
                        {service.triggers.map((item) => <li key={item}>— {item}</li>)}
                      </ul>
                    </div>
                    <div>
                      <h4 className="text-sm font-semibold text-white">Typical deliverables</h4>
                      <ul className="mt-4 space-y-3 text-sm leading-6 text-slate-400">
                        {service.deliverables.map((item) => <li key={item}>— {item}</li>)}
                      </ul>
                    </div>
                    <div className="md:col-span-2 border-t border-white/10 pt-6">
                      <h4 className="text-sm font-semibold text-white">Engagement scope</h4>
                      <p className="mt-3 text-sm leading-6 text-slate-400">{service.scope}</p>
                      <div className="mt-5 flex flex-wrap gap-x-5 gap-y-3">
                        {service.related.map(([label, href]) => (
                          <Link key={href} href={href} className="text-sm font-medium text-slate-300 hover:text-cyan-200">{label} -&gt;</Link>
                        ))}
                      </div>
                    </div>
                  </div>
